...
def trans_str2(trans_list):
    return "JUNK trans str:" + str(len(trans_list))


trans_str = trans_str2
...
def eisen_grammar():
    ...
    rule_call = (pp.ZeroOrMore(loop) + rule_name("rule_name")).setResultsName(
        "rcall", listAllMatches=True
    )
...
def es_call(top, rule, pcall, is_bcall, n):
    ...
    if is_bcall:
        ...
        atr_value = "JUNK shape"
    else:
        call_type = "call"
        atr_name = "rule"
        atr_value = pcall.rule_name
        atr_value = "JUNK rule"
